...
class SummarizeJulesActivitiesTest(unittest.TestCase):
    def summarize(self, activities: list[dict]) -> tuple[int, int, int, str]:
        ...
        agent_epoch, user_epoch, token_epoch, wait_kind = output.split("\t")
        return int(agent_epoch), int(user_epoch), int(token_epoch), wait_kind
