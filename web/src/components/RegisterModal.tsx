...
const MS_PLACEHOLDER = `Один аккаунт на строку, разделяя поля четырьмя дефисами:
...
export function RegisterModal({ open, onClose, onJobFinished }: Props) {
  ...
  const handleSubmit = async () => {
    setSubmitError(null)
    if (!activeProvider) {
      setSubmitError('Не найдены доступные провайдеры регистрации')
      return
    }
    if (!input.trim()) {
      setSubmitError('Пожалуйста, вставьте хотя бы один аккаунт')
      return
    }
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-border">
          <div className="flex items-center gap-2 text-text-primary">
            ...
            <span className="text-[14px] font-semibold tracking-tight">Пакетная регистрация</span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center justify-between gap-2 px-5 py-3 border-t border-border bg-bg-secondary">
          {!job ? (
            <>
              <span className="text-[11px] text-text-muted">Закрытие окна не отменит задачи. Их можно посмотреть в Истории задач.</span>
            ...
            <>
              <span className="text-[11px] text-text-muted tabular-nums">
                {job.state === 'running' ? 'В процессе в фоне...' : 'Завершено'}
              </span>
              <button
                onClick={onClose}
    ...
    <div className="p-5 space-y-4">
      <div>
        <label className="block text-[12px] text-text-secondary mb-1.5 font-medium">
          {provider ? `${provider.display} Учетные данные` : 'Учетные данные'}
        </label>
        <textarea
          value={input}
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="inline-flex items-center gap-1 text-[11px] text-text-muted">
          ...
          Рекомендация по безопасности: ≤ 5
        </span>
      </div>

      <div>
        <label className="block text-[12px] text-text-secondary mb-1.5 font-medium">
          URL Прокси <span className="text-text-muted font-normal">(опционально)</span>
        </label>
        <input
          type="text"
          value={proxy}
          ...
          placeholder={
            globalProxy
              ? `Оставить пустым = использовать глобальный прокси (${globalProxy})`
              : 'Оставить пустым = напрямую; пример: socks5://user:pass@host:port'
          }
        ...
        <div className="text-[11px] text-text-muted mt-1">
          Поддерживаются socks5 / socks5h / http / https. Имеет приоритет; если пусто, то
          {globalProxy ? 'откат к глобальному прокси (включен)' : 'напрямую (глобальный прокси не настроен)'}。
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          ...
          <span className="text-[12px] text-text-muted tabular-nums">
            {done} / {total} · OK {ok} · Ошибка {fail} · Параллельность {job.concurrency}
          </span>
        </div>
        ...
        {!streamConnected && job.state === 'running' && (
          <div className="text-[11px] text-warn flex items-center gap-1">
            <IconAlert size={11} /> Стрим отключен, опрос... {streamError && `(${streamError})`}
          </div>
        )}
      </div>
    ...
    case 'running':
      statusEl = (
        <span className="inline-flex items-center gap-1 text-[11px] text-notion-blue">
          <IconSpinner size={11} className="animate-spin" /> В процессе
        </span>
      )
      break
    case 'ok':
