...
- active sessions инспектируются freshest-update-first; первая session каждого `task_id` становится canonical owner, а остальные получают durable DELETE и не допускаются к approve/scope/recovery/task-owner mutation;
...
- divergence всегда считается от live `base.ref`, а не от snapshot `pull.base.sha`; `comparison.base_commit.sha` становится current-base evidence для branch-update key, progress, recovery и source fingerprints;
- update-branch `422` классифицируется вместе с sanitized GitHub message: `merge conflict between base and head` немедленно включает semantic recovery, а modified-head/unknown validation сохраняет race-safe `head_raced` fallback;
- тот же durable branch update выполняется до recovery и при уже активной Jules session: accepted/raced update подавляет лишний `sendMessage` в текущем цикле, пяти минут ожидания и двух попыток достаточно для bounded handoff, а conflict/error возвращает управление semantic recovery;
- protected-path gate не завершает automerge до recovery routing: он формирует structured failure report, после чего общий quality-stop ставит `needs-quality-fix`, обновляет deduplicated comment и dispatch-ит authoritative reconciler; запрет secret/data/binary/log/workflow/scratch paths при этом остаётся блокирующим;
- quality gate разрешает test-only diff только для явного test-only manifest contract: все non-manifest `allowed_paths` тестовые, goal явно требует tests и каждый acceptance test-specific; наличие runtime acceptance сохраняет operational block;
...
После merge native sync PR #622 дал отдельный early-gate fixture: reconciler run `29306562700` выполнил `update_pr_branch` без Jules recovery и убрал `behind_by`, после чего quality run `29306599931` корректно нашёл `pr_body.txt`, но завершился до label/comment/reconciler steps. Этот live blocker является основанием маршрутизировать protected-path failure через тот же bounded quality recovery path.

После merge #623 recovery session `14674460986994187107` уже вернула PR #622 к product-only diff на head `59425436`, но новый master `2c16af8` снова сделал ветку behind. Quality run `29307014157` увидел файлы нового base как out-of-scope, а active-session path вместо native sync leased duplicate session `17208555400991546940`. Этот fixture фиксирует, что branch update обязан предшествовать recovery в обоих PR paths; механическая синхронизация не является новой задачей для Jules.

Post-merge run `29307525125` выявил более глубокий source-of-truth gap: REST payload PR #622 сохранял `base.sha=a57b66b`, хотя live `master` уже был `6b58840`. Поэтому compare старого SHA с head `a4a61c8` ложно давал `behind_by=0`, reconciler отправил `new_failed_checks` recovery, а Jules создала пустой commit. Compare `master...a4a61c8` корректно показывал `behind_by=2`; этот live fixture требует резолвить current base через `base.ref` и сохранять `comparison.base_commit.sha` во всех durable fingerprints.

После исправления current-base source run `29307857421` впервые дал точный live результат: `update_pr_branch` для PR #622 с `base_sha=6648185`, новый head `2687154`, без `send_recovery`. Quality run `29307880351` затем выявил независимый false positive: задача явно требовала только два unit-теста и разрешала только `tools_test.go`, но слово logging включало operational guard. Exact refs/body #622 проходят после contract-aware test-only исключения, при этом смешанный runtime+test acceptance остаётся блокирующим.

Следующий quality-recovery run `29307892758` показал два active owner одной задачи: sessions `14674460986994187107` и `17208555400991546940` получили одинаковый `new_failed_checks` recovery. Executor уже запрещает duplicate create, но ранее завершённая session может снова стать active после сообщения. Reconciler поэтому обязан выбрать одну freshest session и удалить остальные до любого approve/sendMessage; один `task_id` не может одновременно управлять одной PR-веткой из двух Jules contexts.

После merge dedupe run `29308520419` попытался синхронизировать загрязнённый head `9ffd977`; GitHub вернул `422` и ledger message `merge conflict between base and head`. Старый status-only mapping ошибочно сохранил outcome `head_raced`. Payload-aware classification устраняет ложное ожидание: такой ответ является conflict evidence, тогда как реальный expected-head race по-прежнему не разрешает перетирать параллельный push.
